...
std::string Timestamp::tostring() const {
  ...
  snprintf(buf, 20, "%4d-%02d-%02d %02d:%02d:%02d", tm_time->tm_year + 1900, tm_time->tm_mon + 1, tm_time->tm_mday,
  ...
  for (size_t ii = 0; ii < threadnum; ii++) {
    ...
    threads_.emplace_back([this] {
      printf("create %s thread(%d).\n", threadtype_.c_str(),
             syscall(SYS_gettid));  // 显示线程类型和线程ID。
      ...
      while (stop_ == false) {
        ...
        {  // 锁作用域的开始。 ///////////////////////////////////
          ...
          this->condition_.wait(lock,
                                [this] { return ((this->stop_ == true) || (this->taskqueue_.empty() == false)); });
...
int createtimerfd(int sec = 30) {
  /*
  第一个参数 CLOCK_MONOTONIC：使用单调时间（不受系统时间调整影响）
  第二个参数：
  TFD_CLOEXEC：执行exec时关闭文件描述符
  ...
  */
  ...
  timeout.it_interval.tv_sec = sec;  // 单次触发，无周期性, 设置为2表示首次触发之后每两秒触发一次
...
void EventLoop::setepolltimeoutcallback(std::function<void(EventLoop *)> fn) { epolltimeoutcallback_ = fn; }
...
void EventLoop::queueinloop(std::function<void()> fn) {
  {
    std::lock_guard<std::mutex> gd(task_mtx_);  // 给任务队列加锁。
...
void EventLoop::handletimer() {
  ...
  // 这里不read也没关系，还是定时触发，要把uint64_t的缓冲区撑满也不是一件容易的事
  uint64_t exp;
  read(timerfd_, &exp, sizeof(exp));  // 清空可能积累的事件
  ...
  if (mainloop_) {
    ...
    // 整个循环都要加锁，因为其他地方也会erase conn，比如closecallback，这样迭代器就会失效
    {
...
// 从map中移除conn
void EventLoop::removeconn(int fd) {
  std::lock_guard<std::mutex> gd(mtx_);
  conns_.erase(fd);
}
...
void Acceptor::setnewconnectioncb(std::function<void(std::unique_ptr<Socket>)> fn) { newconnectioncb_ = fn; }
...
void Connection::writecallback() {
  int writen = ::send(fd(), outputbuffer_.data(), outputbuffer_.size(), 0);  // 尝试把outputbuffer_全部发送出去。
...
// 判断TCP连接是否超时（空闲太久）。
bool Connection::timeout(time_t now, int val) { return now - lastatime_.toint() > val; }
...
void TcpServer::closeconnection(spConnection conn) {
  ...
  subloops_[conn->fd() % threadnum_]->removeconn(conn->fd());  // 从loop中删除conn
}
...
EchoServer::EchoServer(const std::string &ip, const uint16_t port, int iothreadnum, int workthreadnum)
  ...
  // tcpserver_.settimeoutcb(std::bind(&EchoServer::HandleTimeOut, this,
  // std::placeholders::_1));
}
...
void EchoServer::HandleNewConnection(spConnection conn) {
  ...
  // printf("EchoServer::HandleNewConnection() thread is
  // %d.\n",syscall(SYS_gettid));
